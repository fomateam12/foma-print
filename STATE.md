...
_Son güncelleme: 2026-09-13 (Claude) — son iş: Leatherette Tumblers alt kategorisi 404 düzeltildi (subId 130 çakışması → 136, çift subId artık build'i durdurur)_
...
## Sırada / açık işler
- **Leatherette Tumblers 404 DÜZELTİLDİ (13 Eyl, dal `claude/leatherette-tumblers-subid-fix`, merge bekliyor):** `/category/drinkware/leatherette-tumblers` 16 Ağu'dan beri "Not found" basıyordu — Patches ayrımı (PR, 16 Ağu) subId 130-134'ü aldı, Leatherette Tumblers tile'ı da 130'daydı; `subSlugBySubId` son yazan kazandığı için 23 LTM SKU sessizce `drinkware/rectangle-patches`'e düştü, tile 0 ürünle silindi. Çözüm: tile + 25 SUB_OVERRIDES satırı subId **136**; `normalizeCatalog` artık ADDED_SUBCATEGORIES'te çift subId görünce build'i düşürüyor (aynı hata 1 Ağu'da 124/Spiral Notebooks ile de olmuştu). Dolu subId'ler: 91–110, 115, 121–124, 130–136. ⛔ www.fomaprint.com Hetzner'dan servis ediliyor (Eymen'in sunucusu, bu makinede erişim yok) — merge Vercel'i düzeltir, canlı alan adı için sunucuda main çekilip build alınmalı.
